...
import sys, urllib2, email, re, csv, StringIO, base64, json, datetime, pprint, time
...
ERROR_TEMP_FAIL = 75
MONGO_HOST = environ.get("MONGO_HOST", "mongo:27017")
MONGO_EXPIRE = environ.get("MONGO_EXPIRE", "60*60*24")
...
if __name__ == "__main__":
    ...
    parser.add_option("-m", "--mongo", dest="do_mongo", action="store_true",help="if present write document to mongodb")

    opt, args = parser.parse_args()

    if not opt.url and not opt.do_print and not opt.do_mongo:
    ...
    try:
        ...
        if opt.do_print:
            ...
            coll = json.loads(json_data)
            coll['_id'] = coll['headers']['message-id']
            collection = coll['to'][0]['email']
            mail_col = db[collection]
            coll['created_at'] = datetime.datetime.utcnow()
            coll['epoch_time'] = int(time.time() * 1000)
            mail_col.ensure_index("created_at", expireAfterSeconds=MONGO_EXPIRE)
            mail_col.ensure_index("epoch_time")
            db[collection].insert(coll)
        else:
